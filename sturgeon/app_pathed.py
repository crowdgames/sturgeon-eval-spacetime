...
class PathCanvas(tkinter.Canvas):
    def __init__(self, parent, cfg, mode, app_state):
    ...
    @staticmethod
    def gen_proc_body(q, cfg, seed_gen, path, text_level):
        ...
        if PathCanvas.is_edge_path(path):
            cfg.custom_constraints.append(util_custom.OutPathConstraint(path, WEIGHT_PATH))
        if text_level is not None:
            cfg.custom_constraints.append(util_custom.OutTextLevelConstraint(text_level, None))
    ...
    def on_timer(self):
        ...
        if self._gen_proc_wanted is not None:
            if self._gen_proc is not None:
                ...
                self._gen_proc = multiprocessing.Process(target=PathCanvas.gen_proc_body, args=(self._gen_proc_q, self._cfg, self._seed_gen, self._path_or_point, self._text_level))
    ...
    def redraw_from_working(self):
        ...
        if self._gen_objective is not None and self._gen_objective > 0:
            self._working_draw.append(self.create_line(xoff + tocvs(0.65), yoff + tocvs(0.65), xoff + tocvs(1.35), yoff + tocvs(1.35), fill='purple', width=3))
            self._working_draw.append(self.create_line(xoff + tocvs(1.35), yoff + tocvs(0.65), xoff + tocvs(0.65), yoff + tocvs(1.35), fill='purple', width=3))

        if self._gen_proc is not None:
            self._working_draw.append(self.create_arc(xoff + tocvs(0.5), yoff + tocvs(0.5), xoff + tocvs(1.5), yoff + tocvs(1.5), outline='purple', width=3, style=tkinter.ARC, start=time.time() * 45.0, extent=300.0))
        ...
        if self._text_level is not None:
            for rr in range(self._rows):
                for cc in range(self._cols):
                    ...
                    if text != util_common.DEFAULT_TEXT:
                        self._text_level_draw.append(self.create_rectangle(tocvs(cc), tocvs(rr), tocvs(cc + 0.5), tocvs(rr + 0.5), fill='cyan', outline='blue'))
                        self._text_level_draw.append(self.create_text(tocvs(cc + 0.25), tocvs(rr + 0.25), text=text, fill='blue', font=('Courier', CELL_SIZE // 2), anchor=tkinter.CENTER))
    ...
    def on_evt_all_export(self, event):
        if self._gen_result is not None:
            ...
            outfile_base = cfg.outfile + '_' + hashlib.md5(json.dumps(app_state).encode('utf-8')).hexdigest()
...
def pathed(cfg, mode, app_state):
    ...
    canvas = PathCanvas(frame, cfg, mode, app_state)
    scroll = False
